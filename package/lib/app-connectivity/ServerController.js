...
class ServerController extends events_1.EventEmitter {
    ...
    onConnectionCreated(clientQuery, clientConnection, downgrade) {
        const { app, os, device, device_id, sdk_version, csr, csr_path, medium, rsocket, } = clientQuery;
    ...
    onSecureConnectionAttempt(clientQuery) {
        ...
        if (clientQuery.medium === 'WWW' || clientQuery.medium === 'NONE') {
            this.flipperServer.registerDevice(new DummyDevice_1.default(this.flipperServer, clientQuery.device_id, `${clientQuery.app} (Unknown Device)`, clientQuery.os));
        }
        const client = {
            os: clientQuery.os,
            ...
            (0, flipper_common_1.reportPlatformFailures)(certificateProvider.processCertificateSigningRequest(clientQuery, unsanitizedCSR, appDirectory), 'processCertificateSigningRequest')
                ...
                if (!result.error) {
                    ...
                    if (device) {
                        ...
                        this.flipperServer.registerDevice(new DummyDevice_1.default(this.flipperServer, deviceId, `${clientQuery.device} via QR Exchange`, clientQuery.os));
                    }
    ...
    removeConnection(id) {
        ...
        if (info) {
            recorder_1.recorder.log(info.client.query, `Disconnected: ${info.client.query.app} on ${info.client.query.device_id}.`);
